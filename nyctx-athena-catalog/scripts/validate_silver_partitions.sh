...
if command -v nyctx-athena-validate >/dev/null 2>&1; then
  exec nyctx-athena-validate "$@"
fi

if command -v uv >/dev/null 2>&1; then
  exec env XDG_CACHE_HOME="${CACHE_ROOT}" UV_CACHE_DIR="${UV_CACHE_ROOT}" \
    uv run --package nyctx-athena-catalog nyctx-athena-validate "$@"
fi

exec env PYTHONPATH="${ATHENA_SRC}${PYTHONPATH:+:${PYTHONPATH}}" \
  python -c 'from nyctx_athena_catalog.cli import validate_partitions_main; validate_partitions_main()' "$@"